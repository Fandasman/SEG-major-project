...
class ApproveTheApplicationAsOTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('accept_applicant',kwargs={'club_id':self.club.id,'member_id':self.member_user.id})


    def test_approve_applicatin_as_O_url(self):
        self.assertEqual(self.url,f'/accept_applicant_as_officer/{self.club.id}/{self.member_user.id}')

    def test_get_approve_application_as_O_is_forbidden(self):
